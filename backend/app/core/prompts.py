...
1. **格式破坏**: 优化后的提示词是否存在严重截断、乱码、或完全无法阅读的格式混乱？
2. **核心意图丢失**: 优化后的提示词是否**完全**丢失了原始提示词的核心任务描述或角色设定？
3. **模板变量丢失**: 原始提示词中的**关键**模板变量（如 `{input}`, `{context}` 等）是否在优化后被删除？
4. **严重偏离**: 优化后的提示词是否与原始任务**完全无关**，或产生了自相矛盾的致命指令？
5. **重复内容**: 优化后的提示词是否存在大量无意义的重复段落（超过3次相同段落）？

## 判断原则（重要！）
请保持**宽容**的审核态度：
- **只有**在发现**本质性破坏**和**输出结构被破坏**时才判定为 `high`（例如：内容严重截断无法使用、出现乱码、核心任务描述完全消失）
- 常规的优化调整（如：措辞变化、结构调整、新增约束或示例）应视为正常优化，不应判为 `high`
- 轻微的表述差异、段落顺序变化、新增内容等属于 `low` 或 `none`
- **当存疑时，倾向于判定为 `low` 而非 `high`**
...
- `severity`: 问题严重程度
  - `none`: 无问题，优化结果完全可靠
  - `low`: 轻微问题，优化结果可接受
  - `high`: 本质性破坏，必须拒绝此优化结果（仅在截断、乱码、核心意图完全丢失时使用）
"""
