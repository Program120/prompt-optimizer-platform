...
### 2. 意图定义表格
用表格列出所有意图标签，必须包含以下几要素, **意图名称要严格遵循原提示词中的意图,不要新增意图**, **除意图名称意外的所有列每次仅允许最小幅改动(不超过多3个新增/删改/调整), 稳步迭代!**：

| 意图名称 | 业务描述 | 业务范围 | 典型 Query | 排除场景 |
|---------|---------|---|-----------|---------|
| 查询订单 | 用户想了解订单状态、物流进度 | - | "我的快递到哪了"、"订单什么时候发货" | 不包含退款、取消等操作性请求 |
| 申请退款 | 用户希望对已购商品申请退货退款 | - | "不想要了想退"、"怎么申请退款" | 不包含仅查询退款进度 |
