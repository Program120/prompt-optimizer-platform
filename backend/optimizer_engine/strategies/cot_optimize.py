...
### 2. 思维链结构
- 若原提示词中包含 `reason` 相关输出要求，在输出意图之前，要求模型说明判断逻辑：
```
判断依据：[分析用户query的关键信息] → 匹配意图：[最终意图标签]
```
- 若原提示词中无 `reason` 相关输出要求，直接按照第3点的推理步骤模板执行即可。
...
Step 5: 输出结果 - 生成标准化JSON输出, (如果原提示词是json字段, 允许可以修改原提示词输出中json字段顺序, 不允许修改输出结构) 
```
