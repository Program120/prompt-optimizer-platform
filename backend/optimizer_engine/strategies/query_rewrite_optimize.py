...
class QueryRewriteOptimizationStrategy(BaseStrategy):
    ...
    def apply(
        ...
        # 构造优化指令
        instruction: str = f"""当前提示词在 Query 预处理方面存在不足，需要统一输入口径，消除口语化和黑话干扰。
...
请按照以下要点完善提示词的 Query 预处理规则部分：

### 1. 明确位置（Module Order: 3）
- **先识别**原提示词中 Query 预处理规则的具体位置。
...
### 6. 格式要求 (Strict Mode)
- **严禁修改其他模块**: 你只能修改"Query 预处理规则"相关的内容。绝对禁止修改前面的约束规则或后面的意图定义等其他模块的内容。
